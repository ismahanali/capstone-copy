...
const ResetPassword = () => {
  ...
  const onFormSubmit = async (values, actions) => {
    ...
    if (apiResponse.status === 1) {
      navigate("/login");
      toast(
        "Your password has been rest successfully. Please proceed to login"
      );
    } else {
      actions.resetForm();
  ...
  const ResetPasswordSchema = Yup.object().shape({
    password: Yup.string()
      .matches(
        /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})/,
        "Must Contain 8 Characters, One Uppercase, One Lowercase, One Number and One Special Case Charater"
      )
